...
class IntelligentQueryProcessor:
    ...
    def _calculate_percentage_fallback(self, ws, query_components: Dict[str, Any]) -> Optional[str]:
        """Fallback percentage calculation by scanning sheet - IMPROVED ACCURACY"""
        try:
            # Scan for relevant rows with better filtering
            numerator_rows = []
            denominator_rows = []
            ...
            for row in range(1, min(100, ws.max_row + 1)):
                for col in range(1, min(8, ws.max_column + 1)):
                    ...
                    if cell.value and isinstance(cell.value, str):
                        ...
                        # Check for numerator terms (more specific)
                        if any(term in cell_text for term in ['indirect', 'overhead', 'insurance']):
                            # Avoid double-counting
                            if row not in numerator_rows:
                                numerator_rows.append(row)
                        
                        # Check for denominator terms (more specific)
                        if any(term in cell_text for term in ['cost', 'expense', 'operating expense', 'total expense']):
                            if 'income' not in cell_text and 'revenue' not in cell_text and 'gross' not in cell_text:
                                if row not in denominator_rows:
                                    denominator_rows.append(row)
            
            if not numerator_rows or not denominator_rows:
                return None
            
            # Calculate percentages for available columns with validation
            percentages = {}
            ...
            for col in range(1, min(20, ws.max_column + 1)):
                try:
                    ...
                    if denominator_total > 0 and numerator_total > 0:
                        ...
                        # Sanity check: percentage should be reasonable (0-1000%)
                        if 0 <= percentage <= 1000:
                            # Get column label for context
                            col_label = ws.cell(1, col).value or f"Column {col}"
                            percentages[col_label] = percentage
                except (TypeError, ValueError):
                    continue
            
            if percentages:
                # Return the most reasonable percentage found
                valid_percentages = {k: v for k, v in percentages.items() if 0 <= v <= 100}
                
                if valid_percentages:
                    # Prefer percentages under 100%
                    reasonable_percentages = {k: v for k, v in valid_percentages.items() if v <= 100}
                    if reasonable_percentages:
                        period, percentage = list(reasonable_percentages.items())[0]
                        return f"{percentage:.1f}% in {period}"
                    else:
                        period, percentage = list(valid_percentages.items())[0]
                        return f"{percentage:.1f}% in {period}"
                else:
                    # If all percentages are over 100%, return the lowest one
                    period, percentage = min(percentages.items(), key=lambda x: x[1])
                    return f"{percentage:.1f}% in {period}"
    ...
    def _is_time_period_column(self, col_label: str, query_components: Dict[str, Any]) -> bool:
        """Check if a column represents a time period - IMPROVED ACCURACY"""
        if not col_label:
            return False
        
        label_lower = col_label.lower()
        
        # Check for specific periods mentioned in query with higher priority
        ...
        if not query_components['time_periods']:
            # Only return true for clear time indicators
            time_patterns = [
            ...
            # Don't include years unless they're specifically requested
    ...
    def _format_forecast_series_response(self, query_components: Dict[str, Any], results: Dict[str, Any]) -> str:
        """Format response for forecast series queries - IMPROVED ACCURACY"""
        
        # Try multiple approaches to find forecast data
        for approach in ['table_data', 'data_points', 'sheet_scan']:
            if approach == 'table_data':
                # Approach 1: Use detected table structures
                for sheet_name, sheet_data in results.items():
                    ...
                    for table_data in sheet_data['tables']:
                        forecast_result = self._extract_forecast_from_table(table_data, query_components)
                        if forecast_result:
                ...
                for sheet_name, sheet_data in results.items():
                    ...
                    forecast_result = self._extract_forecast_from_data_points(sheet_data['data_points'], query_components)
                    if forecast_result:
                        return forecast_result
                ...
                for sheet_name, sheet_data in results.items():
                    ws = self.wb[sheet_name]
                    forecast_result = self._extract_forecast_from_sheet(ws, query_components)
                    if forecast_result:
                        return forecast_result
        
        return "Could not find forecast data"
    
    def _extract_forecast_from_table(self, table_data: Dict[str, Any], query_components: Dict[str, Any]) -> Optional[str]:
        """Extract forecast data from table structure"""
        try:
            # Find forecast-related rows for the metric
            forecast_rows = []
            for row, label in table_data['row_labels'].items():
                label_lower = label.lower()
            ...
            # Find forecast columns - prioritize the requested time period
            forecast_cols = []
            requested_year = None
            ...
            for period in query_components['time_periods']:
                if re.match(r'20\d{2}', period):
                    requested_year = period
                    break
            
            # Find columns that match the requested time period
            for col, col_label in table_data['headers'].items():
            ...
            # Sort by priority and extract data
            forecast_cols.sort(key=lambda x: x[2])  # Sort by priority
            ...
            # Extract data from priority columns
            for col, col_label, priority in forecast_cols:
                total_value = 0
                ...
                if total_value > 0:
                    formatted_value = self._format_financial_value(total_value)
            ...
            logger.warning(f"Error extracting forecast from table: {e}")
        
        return None
    
    def _extract_forecast_from_data_points(self, data_points: List[Dict[str, Any]], query_components: Dict[str, Any]) -> Optional[str]:
        """Extract forecast data from data points"""
        try:
            # Group by time period and check for forecast indicators
            period_data = {}
            requested_year = None
            ...
            # Check if we have forecast data for the requested period
            if requested_year:
                forecast_data = {}
                ...
                for period, points in period_data.items():
                    ...
                    # Check if this period is for the requested year
                    if requested_year in period_lower or any(month in period_lower for month in ['jan', 'feb', 'mar', 'apr', 'may', 'jun', 'jul', 'aug', 'sep', 'oct', 'nov', 'dec']):
                        total_value = sum(abs(dp.get('value', 0)) for dp in points)
                        if total_value > 0:
                            forecast_data[period] = total_value
                
                if forecast_data:
                    return self._format_forecast_output(forecast_data, query_components)
            
        except Exception as e:
            logger.warning(f"Error extracting forecast from data points: {e}")
        
        return None
    
    def _extract_forecast_from_sheet(self, ws, query_components: Dict[str, Any]) -> Optional[str]:
        """Extract forecast data by scanning sheet directly"""
        try:
            # Find metric rows
            metric_rows = []
            for row in range(1, min(100, ws.max_row + 1)):
            ...
            # Find forecast columns for the requested time period
            requested_year = None
            for period in query_components['time_periods']:
                if re.match(r'20\d{2}', period):
                    requested_year = period
            ...
            for col in range(1, min(30, ws.max_column + 1):
                ...
                if header_cell.value and isinstance(header_cell.value, str):
                    ...
                    # Check if this column is for the requested year
                    if requested_year and requested_year in header_text:
                        forecast_cols.append((col, header_cell.value))
                    # Check for forecast indicators
                    elif any(term in header_text for term in ['forecast', 'budget', 'plan']):
                        if requested_year and requested_year in header_text:
                            forecast_cols.append((col, header_cell.value))
            ...
            for col, col_label in forecast_cols:
                ...
                if total_value > 0:
                    forecast_data[col_label] = total_value
            
            if forecast_data:
                return self._format_forecast_output(forecast_data, query_components)
            
        except Exception as e:
            logger.warning(f"Error extracting forecast from sheet: {e}")
        
        return None
    
    def _format_forecast_output(self, forecast_data: Dict[str, float], query_components: Dict[str, Any]) -> str:
        ...
        result_lines = [f"{company} {metric} forecasts:"]
        
        # Sort by time if possible
        try:
            sorted_periods = sorted(forecast_data.items(), key=lambda x: self._extract_time_order(x[0]))
        except:
            sorted_periods = sorted(forecast_data.items())
